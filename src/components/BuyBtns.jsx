...
const BuyBtns = ({ id, }) => {
  ...
  return (
    <div className={" buy-btns   my-3 flex flex-col items-center gap-2 "}>
      <button onClick={() => userAuthed ? handleAddToCart(id) : handleRedirect() } onSubmit={() => ""} disabled={isOrdered} className={(isOrdered ? "bg-slate-400 " : " bg-orange-600 active:bg-orange-500  ") + "buy-now w-10/12 h-max py-1 px-2  max-w-xs flex justify-center rounded-2xl text-white text-lg font-[arial] font-thin cursor-pointer transition-transform"}>
        <span className={" flex"}>
