  ...
  const BreadCrumbs =()=>{
    ...
    return(
      <span id="crumbs" className="crumbs flex items-center ">
        ...
        {
          crumbs?.map((cr,x)=> 
          <Link to={cr.path || "/"}  key={x} className={"w-full flex items-center text-[100%] text-center font-semibold  " + (x === crumbs.length - 1 ? "  pointer-events-none text-slate-500" : " text-orange-500 ")}>
            <span className="w-auto max-w-[150px] h-min flex items-center truncate ">
              {cr.text}
            </span>
